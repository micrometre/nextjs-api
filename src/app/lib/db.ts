...
export const getUserByGoogleId = (googleId: string): DbUser | undefined => {
    return db.prepare('SELECT * FROM users WHERE googleId = ?').get(googleId) as DbUser;
}

export const createUser = (name: string, email: string, googleId?:string): number => {
    const stmt = db.prepare('INSERT INTO users (name, email, googleId) VALUES (?, ?, ?)');
